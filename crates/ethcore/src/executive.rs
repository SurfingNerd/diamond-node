...
pub fn contract_address(
    ...
    match address_scheme {
        ...
        CreateContractAddress::FromSenderSaltAndCodeHash(salt) => {
            ...
            buffer[0] = 0xff;
            let _ = &mut buffer[1..(1 + 20)].copy_from_slice(&sender[..]);
            let _ = &mut buffer[(1 + 20)..(1 + 20 + 32)].copy_from_slice(&salt[..]);
            let _ = &mut buffer[(1 + 20 + 32)..].copy_from_slice(&code_hash[..]);
            (From::from(keccak(&buffer[..])), Some(code_hash))
        }
        CreateContractAddress::FromSenderAndCodeHash => {
            let code_hash = keccak(code);
            let mut buffer = [0u8; 20 + 32];
            let _ = &mut buffer[..20].copy_from_slice(&sender[..]);
            let _ = &mut buffer[20..].copy_from_slice(&code_hash[..]);
            (From::from(keccak(&buffer[..])), Some(code_hash))
        }
    }
}
