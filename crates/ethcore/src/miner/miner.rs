...
impl Miner {
    ...
    fn create_open_block<'a, C>(
        &self,
        chain: &'a C,
    ) -> Option<(OpenBlock<'a>, Vec<SignedTransaction>)>
    where
        ...
        C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
    {
        ...
        let (mut open_block, original_work_hash, engine_txs) = {
                ...
                .get_pending_if(|b| b.header.parent_hash() == &best_hash)
            {
                ...
                None => {
                    // Do not create block if miner should not prepare blocks.
                    if !self.engine.should_miner_prepare_blocks() {
                        return None;
                    }
                    ...
                    let (block, transactions) = self.create_open_block(chain)?;
                    (block, last_work_hash, transactions)
                }
            }
        };
    ...
    where
        C: BlockChain + CallContract,
        I: IntoIterator<Item = SignedTransaction>,
    {
